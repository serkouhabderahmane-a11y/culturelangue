(function () {
  ...
  window.translations = {
    fr: {
      ...
      'admin.sidebar.lessons': 'Le\u00e7ons',
      'admin.sidebar.sessions': 'S\u00e9ances',
      ...
      'admin.lessons.management': 'Gestion des le\u00e7ons',
      'admin.sessions.title': 'S\u00e9ances',
      'admin.sessions.management': 'Gestion des s\u00e9ances',
    ...
    en: {
      ...
      'admin.lessons.management': 'Lesson management',
      'admin.sessions.title': 'Sessions',
      'admin.sessions.management': 'Session management',
      ...
      'teacher.portal.students.filter.intensive': 'Intensive Program',
  ...
  function init() {
    ...
    /* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
       UPCOMING LESSONS (public homepage)
       â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
    var lessonsCache = null;

    window.loadUpcomingLessons = function () {
      var container = document.getElementById('upcoming-lessons');
      if (!container) return;

      function render(lessons) {
        if (!lessons || !lessons.length) {
          container.innerHTML = '<div class="ph-upcoming-empty">Aucun cours Ã  venir pour le moment.</div>';
          return;
        }
        var lang = window.currentLang === 'en' ? 'en' : 'fr';
        var dateFmt = new Intl.DateTimeFormat(lang === 'fr' ? 'fr-CA' : 'en-CA', {
          weekday: 'long', day: 'numeric', month: 'long'
        });

        var html = '';
        lessons.forEach(function (lesson) {
          var service = lesson.service || {};
          var teacher = lesson.teacher || {};
          var serviceName = lang === 'en' && service.name_en ? service.name_en : (service.name_fr || service.slug || '');
          var teacherName = [teacher.first_name, teacher.last_name].filter(Boolean).join(' ') || 'Cultulangues';
          var dateParts = String(lesson.date || '').slice(0, 10).split('-');
          var d = dateParts.length === 3 ? new Date(+dateParts[0], +dateParts[1] - 1, +dateParts[2]) : null;
          var day = d && !isNaN(d.getTime()) ? dateFmt.format(d) : (lesson.date || '').slice(0, 10);
          var startTime = String(lesson.start_time || '').slice(0, 5);
          var endTime = String(lesson.end_time || '').slice(0, 5);
          var slug = service.slug || '';

          html += '<a class="ph-upcoming-card" href="' +
            (slug ? 'booking.html?course=' + encodeURIComponent(slug) : 'pages/contact.html') + '">' +
            '<div class="ph-upcoming-day">' + day + '</div>' +
            '<div class="ph-upcoming-body">' +
              '<h4 class="ph-upcoming-title">' + serviceName + '</h4>' +
              '<div class="ph-upcoming-meta">' +
                '<span>ðŸ• ' + (startTime || '') + 'â€“' + (endTime || '') + '</span>' +
                '<span>ðŸ‘©â€ðŸ« ' + teacherName + '</span>' +
                (lesson.room ? '<span>ðŸ“ ' + lesson.room + '</span>' : '') +
              '</div>' +
            '</div>' +
            '<span class="ph-upcoming-cta">RÃ©server â†’</span>' +
          '</a>';
        });
        container.innerHTML = html;
      }
      ...
      fetch('/api/v1/lessons')
        ...
        .then(function (payload) {
          var lessons = payload && payload.success ? payload.data : [];
          lessonsCache = lessons;
          render(lessons);
        })
        .catch(function () {
          container.innerHTML = '<div class="ph-upcoming-empty">Impossible de charger les cours pour le moment. Veuillez rÃ©essayer plus tard.</div>';
        });
    };
